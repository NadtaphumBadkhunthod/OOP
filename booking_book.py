...
class BookStock:
    ...
    def get_book_list(self,activity_type : ActivityType):
        if activity_type.value == "BookingBook":
    ...
    def remove_book(self,book):
        if isinstance(book,Book):
            if book.activity_type == "BookingBook":
                self.__booking_book_list.remove(book)
    ...
    def search_book_available(self,bookname,activity_type):
        if activity_type == "BookingBook":
            target_list = self.__booking_book_list
...
class Customer:
    ...
    def check_quota(self): 
        return len([selected for selected in self.__selected_list if isinstance(selected,Book) and selected.book_info.activity_type.value == "BookingBook"]) < (4 - self.__rental_quota)
...
class BookingBook(Purchase):
    def __init__(self, order: list[Book], reservation_date):
        super().__init__(order)
        self.__reservation_date = reservation_date
        # คำนวณ pickup_deadline: วันที่หนังสือพร้อม (available_date) + 3 วัน
        self.__pickup_deadline = self._order[0].available_date + timedelta(days=3)
    ...
    def total_price(self):
        # รวมราคาหนังสือทุกเล่มในรายการจองนี้
        return sum(book.price for book in self._order)

    def check_pickup_deadline(self):
        return datetime.now() > self.__pickup_deadline
...
class Order:
    ...
    def info(self):
        ...
        booking_book_info = [format_book(booking_book) for booking_book in self.__booking_book.get_order]
        return {"Booking Book" : booking_book_info}
    ...
    @booking_book.setter
    def booking_book(self,booking_book : BookingBook):
        self.__booking_book = booking_book
    ...
    def calculate_subtotal(self):
        return self.__purchase_book.calculate_subtotal()
...
class Payment:
    ...
    def calculate_net_amount(self):
        self.__net_amount = self.__order.booking_book.calculate_subtotal() + self.__order.purchase_book.calculate_subtotal() + self.__upgrade_delta - self.__discount_amount + self.__base_fee + self.__penalty_fee
        return self.__net_amount
...
class Transaction:
    ...
    def sync_payment_with_activity(self,bookingbook : BookingBook):
        return True
...
class System:
    ...
    def checkout(self,customer:Customer,staff:Staff,payment_method : MethodPayment):
        ...
        booking_list = [selected for selected in selected_list if isinstance(selected, Book) and selected.book_info.activity_type.value == "BookingBook"]
        ...
        if isinstance(customer,Member):
            if len(booking_list) > customer._Member__booking_book_quota:
                raise HTTPException(status_code=400, detail="จำนวนเล่มที่จองเกินโควต้า")

        order.booking_book = BookingBook(booking_list)
...
async def lifespan(app: FastAPI):
    ...
    # Create_Book
    create_book("How to learn OOP","How to learn OOP","Sixsax",TypeBook.Education,12,ActivityType.BookingBook,1,date.today().strftime("%d/%m/%Y"))
    create_book("How to learn OOP 2","How to learn OOP","Sixsax",TypeBook.Education,12,ActivityType.BookingBook,2,date.today().strftime("%d/%m/%Y"))
    create_book("IDK","IDK","Sixsax",TypeBook.Historical,10,ActivityType.BookingBook,1,date.today().strftime("%d/%m/%Y"))
    create_book("IDK 2","IDK","Sixsax",TypeBook.Historical,10,ActivityType.BookingBook,5,date.today().strftime("%d/%m/%Y"))
...
if __name__ == "__main__":
    uvicorn.run("booking_book:app", host="127.0.0.1", port=8000, log_level="info",reload=True)